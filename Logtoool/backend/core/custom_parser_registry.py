...
def _score_module(module: Any, sample_text: str) -> float:
    ...
    that got a non-null `action` -- used only to break ties when more than
    one module's detect() matches (see the Debit-family ambiguity note in
    ...
    try:
        ...
        if not records:
            return 0.0
        successful = sum(1 for r in records if r.get("action"))
