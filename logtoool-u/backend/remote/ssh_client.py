...
Host key verification uses trust-on-first-use (TOFU): the first successful
...
def _connect(
    ...
    try:
        if auth_type == "password":
            client.connect(
                hostname=host, port=port, username=username, password=secret, timeout=timeout
            )
        elif auth_type == "key":
            ...
            for key_cls in (paramiko.Ed25519Key, paramiko.RSAKey, paramiko.ECDSAKey):
                try:
                    ...
                    continue
            if pkey is None:
                raise SSHConnectionError("Private key could not be parsed (tried Ed25519/RSA/ECDSA formats)")
    ...
    client._tofu_fingerprint = presented_fingerprint  # stashed for the caller to persist on first connect
    return client


def test_connection(
    host: str, port: int, username: str, auth_type: str, secret: str, expected_fingerprint: Optional[str]
) -> str:
...
def _walk_sftp(sftp, path: str, recursive: bool, allowed_extensions: Set[str], out: List[RemoteFileInfo]) -> None:
    for entry in sftp.listdir_attr(path):
        ...
        if ext in allowed_extensions:
            out.append(RemoteFileInfo(path=entry_path, mtime=float(entry.st_mtime), size=int(entry.st_size)))
