"""
API-level tests for /api/debit-portal/* -- same fixture pattern as
...
"Log Tracker No" content (see custom_parser_registry.py's ambiguity
scoring) -- forcing keeps the test deterministic.
"""
import io
...
PROFILE_NAME = "Debit Portal Log (Transactions + Errors)"
...
SAMPLE_LOG = (
    f'{_T0.strftime("%-m/%-d/%Y %I:%M:%S %p")} Log Tracker No: SU1 => Debit Request : '
    '{"TransactionId": "TXN1", "IssuerId": "ISS1", "Status": "SUCCESS", '
    '"MerchantInfo": {"MerchantName": "Store A"}, "TransactionInfo": {"TransactionCurrency": "840"}}\n'
)


def test_unauthenticated_request_rejected(client):
    assert client.get("/api/debit-portal/summary").status_code == 401


def test_summary_endpoint_with_no_data(admin_client):
    resp = admin_client.get("/api/debit-portal/summary")
    assert resp.status_code == 200
    assert resp.json()["status"] == "no_data"


def test_summary_endpoint_after_ingestion(admin_client):
    files = {"file": ("debit.log", io.BytesIO(SAMPLE_LOG.encode()), "application/octet-stream")}
...
def test_invalid_lookback_hours_rejected(admin_client):
    resp = admin_client.get("/api/debit-portal/summary?lookback_hours=0")
