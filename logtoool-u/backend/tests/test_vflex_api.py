"""
API-level tests for /api/vflex/* -- same fixture pattern as
...
PROFILE_NAME = "VFlex StepUp/Bank API/OTP Log (Transaction Correlation)"
...
SAMPLE_LOG = (
    f'{_T0.strftime("%-m/%-d/%Y %I:%M:%S %p")} Log Tracker No: IA1 => Success Response from Bank API Call : '
    '{"success": true, "corelationId": "COR1"}\n'
    f'{_T1.strftime("%-m/%-d/%Y %I:%M:%S %p")} Log Tracker No: IA1 => OTP Processed Successfully\n'
)


def test_unauthenticated_request_rejected(client):
    assert client.get("/api/vflex/summary").status_code == 401


def test_summary_endpoint_with_no_data(admin_client):
    resp = admin_client.get("/api/vflex/summary")
    assert resp.status_code == 200
    assert resp.json()["status"] == "no_data"


def test_summary_endpoint_after_ingestion(admin_client):
    files = {"file": ("vflex.log", io.BytesIO(SAMPLE_LOG.encode()), "application/octet-stream")}
    ...
    assert body["bank_api_success_count"] == 1


def test_invalid_lookback_hours_rejected(admin_client):
    resp = admin_client.get("/api/vflex/summary?lookback_hours=0")
