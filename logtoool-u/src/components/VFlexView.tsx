...
import {
  Smartphone, RefreshCw, Info, Layers, Store, Building2, AlertOctagon, ShieldCheck, ShieldAlert, Landmark,
} from 'lucide-react';
...
export const VFlexView: React.FC = () => {
  const [report, setReport] = useState<VFlexSummary | null>(null);
  ...
  const fetchReport = async (hours: number) => {
    ...
    setError(null);
    try {
      const data = await api.get<VFlexSummary>(`/api/vflex/summary?lookback_hours=${hours}`);
      setReport(data);
    } catch (err) {
  ...
  if (loading && !report) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-slate-400">
        <Smartphone className="w-10 h-10 animate-pulse text-blue-500 mb-3" />
        <p className="text-sm font-medium">Analyzing VFlex activity…</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-200 p-5 rounded-xl shadow-2xs">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">
          Issuer, status, bank-operation, and OTP-channel distribution for the VFlex StepUp/Bank API/OTP log stream.
        </p>
      </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-8 text-center">
          ...
          <p className="text-sm font-medium text-slate-500">{report.message || 'No VFlex activity found in this window.'}</p>
        </div>
      </div>
    );
  }
  ...
  const bankOperationData = toChartData(report.by_bank_operation);
  const channelData = toChartData(report.by_channel);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-slate-500 mb-2">
            <span className="text-xs font-bold uppercase tracking-wider">Tracker Records</span>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-slate-500 mb-2">
            <span className="text-xs font-bold uppercase tracking-wider">Bank API Success</span>
            <Landmark className="w-4 h-4 text-blue-600" />
          </div>
          <div className="text-2xl font-extrabold text-slate-900">{report.bank_api_success_count}</div>
          <div className="text-[11px] text-slate-500 mt-1">of {report.total_records} records</div>
        </div>
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-slate-500 mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-2xs">
          ...
          {bankOperationData.length === 0 ? (
            <div className="h-64 flex items-center justify-center text-xs text-slate-400 italic">No bank operation data in this window.</div>
          ) : (
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={bankOperationData}
                    dataKey="value"
                    nameKey="name"
                    cx="50%"
                    cy="50%"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-2xs">
          ...
          {channelData.length === 0 ? (
            <div className="h-64 flex items-center justify-center text-xs text-slate-400 italic">No channel data in this window.</div>
          ) : (
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={channelData} margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
                  ...
                  <YAxis stroke="#8892A1" fontSize={11} allowDecimals={false} />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Bar dataKey="value" name="Records" fill="#04ADA4" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
