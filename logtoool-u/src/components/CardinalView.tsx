...
import {
  ShieldQuestion, RefreshCw, Info, Layers, Store, Building2, AlertOctagon, ShieldCheck, ShieldAlert, Radio,
} from 'lucide-react';
...
export const CardinalView: React.FC = () => {
  ...
  const fetchReport = async (hours: number) => {
    ...
    setError(null);
    try {
      const data = await api.get<CardinalSummary>(`/api/cardinal/summary?lookback_hours=${hours}`);
      setReport(data);
    } catch (err) {
      setError(err instanceof ApiError ? err.detail : 'Failed to load Cardinal report');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading && !report) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-slate-400">
        <ShieldQuestion className="w-10 h-10 animate-pulse text-blue-500 mb-3" />
        <p className="text-sm font-medium">Analyzing Cardinal activity…</p>
      </div>
    );
  }
  ...
  if (error || !report) {
    return (
      <div className="p-6 bg-rose-50 border border-rose-200 rounded-xl text-rose-800">
        <p className="font-semibold text-sm">Failed to load Cardinal data</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-slate-500 mb-2">
            <span className="text-xs font-bold uppercase tracking-wider">Flows</span>
            <ShieldQuestion className="w-4 h-4 text-blue-600" />
          </div>
          <div className="text-2xl font-extrabold text-slate-900">{report.total_flows}</div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          ...
          <div className="text-[11px] text-slate-500 mt-1">of {report.total_flows} flows</div>
        </div>
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-slate-500 mb-2">
          ...
          <div className="text-[11px] text-slate-500 mt-1">flows with integrity warnings</div>
        </div>
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-slate-500 mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-2xs">
          ...
          {oobStatusData.length === 0 ? (
            <div className="h-64 flex items-center justify-center text-xs text-slate-400 italic">No OOB activity in this window.</div>
          ) : (
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={issuerData} margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
                  ...
                  <Tooltip contentStyle={tooltipStyle} />
                  <Bar dataKey="value" name="Flows" fill="#052460" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-2xs">
          ...
          {bankOrgData.length === 0 ? (
            <div className="h-64 flex items-center justify-center text-xs text-slate-400 italic">No bank org data in this window.</div>
          ) : (
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={bankOrgData} margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E7E7E7" />
                  <XAxis dataKey="name" stroke="#8892A1" fontSize={10} angle={-20} textAnchor="end" interval={0} height={50} />
                  <YAxis stroke="#8892A1" fontSize={11} allowDecimals={false} />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Bar dataKey="value" name="Flows" fill="#3C4B72" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          <div style={{ height: Math.max(220, merchantData.length * 34) }} className="w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={merchantData} layout="vertical" margin={{ top: 10, right: 20, left: 10, bottom: 10 }}>
                ...
                <Tooltip contentStyle={tooltipStyle} />
                <Bar dataKey="value" name="Flows" fill="#FF8800" radius={[0, 6, 6, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
