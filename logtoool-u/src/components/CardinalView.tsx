...
import {
  ShieldQuestion, RefreshCw, Info, Layers, Store, Building2, AlertOctagon, ShieldCheck, ShieldAlert, Radio, Coins,
} from 'lucide-react';
...
export const CardinalView: React.FC = () => {
  ...
  const fetchReport = async (r: DateRangeValue, m: string) => {
    ...
    try {
      ...
      const data = await api.get<CardinalSummary>(`/api/cardinal/summary?${params}`);
      setReport(data);
